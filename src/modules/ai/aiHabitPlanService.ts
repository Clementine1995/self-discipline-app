  ...
  const planBuilders: Record<GoalCategory, () => AiHabitPlanSuggestion[]> = {
    fitness: () => [
      createSuggestion('warmup', '先启动', '用很低门槛建立开始动作，避免一上来就靠意志力硬顶。', {
        name: '运动前热身 3 分钟',
        reminderTime: '19:30',
        rewardText: '连续完成 7 天，允许安排一次散步或喜欢的健康餐，这是守规则换来的。',
        punishmentText: '失败达到阈值后，第二天只做 3 分钟拉伸，把欠下的节奏补回来。',
      }),
      createSuggestion('core', '主任务', '把目标压成每天能重复的核心动作。', {
        name: '完成一次轻量运动',
        reminderTime: '20:00',
        rewardText: '连续完成 7 天，奖励一件运动小装备或一次批准过的休息。',
        punishmentText: '失败达到阈值后，暂停加量，先把最低版本做完。',
      }),
      createSuggestion('review', '收尾复盘', '记录状态能帮助后续调整运动量和提醒时间。', {
        name: '记录今天运动感受',
        reminderTime: '21:30',
        rewardText: '连续记录 7 天，回看一次自己到底有没有变强。',
        punishmentText: '失败达到阈值后，只写一句话也算完成，但不准空着。',
      }),
    ],
    reading: () => [
      createSuggestion('starter', '先翻开', '阅读习惯最怕启动成本，先把动作缩到不会抗拒。', {
        name: '读书 5 分钟',
        reminderTime: '21:00',
        rewardText: '连续完成 7 天，奖励自己买一本真正想读的书。',
        punishmentText: '失败达到阈值后，第二天只读 2 页，读完再休息。',
      }),
      createSuggestion('note', '留下痕迹', '一句笔记比读很多但没记住更容易形成反馈。', {
        name: '写一句阅读笔记',
        reminderTime: '21:20',
        rewardText: '连续记录 7 天，整理一页自己的摘录，证明不是白读。',
        punishmentText: '失败达到阈值后，只写关键词也算完成，但不准跳过。',
      }),
      createSuggestion('prepare', '提前准备', '把书放在看得见的位置，减少明天的启动阻力。', {
        name: '睡前准备明天要读的书',
        reminderTime: '22:30',
        rewardText: '连续准备 7 天，给阅读区添一个小物件。',
        punishmentText: '失败达到阈值后，第二天直接把书放到桌面，先把借口堵住。',
      }),
    ],
    sleep: () => [
    ...
    study: () => [
      createSuggestion('focus', '最小学习块', '学习目标先拆成短时间专注块，降低拖延概率。', {
        name: '专注学习 15 分钟',
        reminderTime: '20:30',
        rewardText: '连续完成 7 天，奖励一次批准过的娱乐时间。',
        punishmentText: '失败达到阈值后，第二天只做 5 分钟启动版，但必须开始。',
      }),
      createSuggestion('output', '输出一点', '输出能确认今天不是只停留在想学。', {
        name: '写 3 行学习小结',
        reminderTime: '21:00',
        rewardText: '连续总结 7 天，整理一份自己的知识清单，给自己一点证据。',
        punishmentText: '失败达到阈值后，只写一个关键词也算完成，但别空着。',
      }),
      createSuggestion('prepare', '明天起步', '提前定好下一步，明天更容易开始。', {
        name: '列出明天学习第一步',
        reminderTime: '22:00',
        rewardText: '连续准备 7 天，减少一次临时焦虑。',
        punishmentText: '失败达到阈值后，只写一个最小动作，别把明天也交给混乱。',
      }),
    ],
    mindfulness: () => [
    ...
    generic: () => [
      createSuggestion('starter', '最低版本', '先设计一个不会被轻易放弃的每日动作。', {
        ...
        punishmentText: '失败达到阈值后，第二天只做最小版本，但不准跳过。',
      }),
      createSuggestion('record', '记录反馈', '每天留下一句反馈，后续才能判断目标是否合适。', {
        name: '写一句目标复盘',
        reminderTime: '21:30',
        rewardText: '连续记录 7 天，回看自己的坚持证据。',
        punishmentText: '失败达到阈值后，只写一个关键词也算完成，但别让复盘空白。',
      }),
      createSuggestion('prepare', '准备明天', '提前准备能减少明天开始时的阻力。', {
        name: '准备明天的第一步',
        reminderTime: '22:00',
        rewardText: '连续准备 7 天，奖励一次不内疚的收尾休息。',
        punishmentText: '失败达到阈值后，第二天只完成一个准备动作，先把开头抢回来。',
      }),
    ],
  };
...
const buildSummary = (goal: string, count: number, toneId: ToneId) => {
  if (!goal) {
    return '先输入一个长期目标，我会把它拆成几个每天逃不掉的小任务。';
  }

  if (toneId === 'rational') {
    return `已把目标拆成 ${count} 个每日任务建议，优先降低启动成本并锁定执行。`;
  }

  if (toneId === 'coach') {
    return `目标别停在口号里。先从这 ${count} 个动作开始执行，别给拖延留空间。`;
  }

  return `我先把它拆成 ${count} 个小任务，轻一点，但每天都要往前挪。`;
};
